...
while True:
    ...
    for i in range(10):
        if i in dire_turn:
            try:
                team1.append(pickList[i] + ".mov")
            except:
                pass
        else:
            try:
                team2.append(pickList[i] + ".mov")
            except:
                pass
    if first_team == "rad":
        rad_pick = team1
    ...
    for i in range(len(banList)):
        if first_team == "rad":
            if i % 2 == 0:
                rad_ban.append(banList[i])
            else:
                dire_ban.append(banList[i])
        else:
            if i % 2 == 1:
                rad_ban.append(banList[i])
            else:
    ...
    if isBan and len(banList) < 14:
        ...
        if teamBanning == "dire":
            dire_ban.append("banning.mp4")
        else:
            rad_ban.append("banning.mp4")
    else:
        teamPicking = team_picking(picked, first_team)
        if teamPicking == "dire":
            dire_pick.append("picking.mp4")
        else:
            rad_pick.append("picking.mp4")
    return_dict["radiant_ban"] = fill_list(rad_ban, "ban")
    return_dict["dire_ban"] = fill_list(dire_ban, "ban")
    return_dict["radiant_pick"] = fill_list(rad_pick, "pick")
    return_dict["dire_pick"] = fill_list(dire_pick, "pick")
